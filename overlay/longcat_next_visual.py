...
try:
    ...
    ImageRefinerContainer = None
    RefinerImageProcessor = None
    RefinerPipeline = None
    de_transform = None
    tensor2pil = None
...
class VisualEncoder(Qwen2_5_VisionTransformerPretrainedModel):
    ...
    def __init__(self, config):
        # Convert DictConfig to a proper HF VisionConfig if needed
        from transformers import PretrainedConfig
        if not isinstance(config, PretrainedConfig):
            from transformers.models.qwen2_5_vl.configuration_qwen2_5_vl import Qwen2_5_VLVisionConfig
            import inspect
            sig = inspect.signature(Qwen2_5_VLVisionConfig.__init__)
            cfg_dict = vars(config) if hasattr(config, '__dict__') else config
            valid_keys = {k: v for k, v in cfg_dict.items() if isinstance(k, str) and k in sig.parameters}
            config = Qwen2_5_VLVisionConfig(**valid_keys)
...
class LongcatNextVisualTokenizer(nn.Module):
    ...
    def lazy_decode_and_save(self, visual_ids, tokens_h, tokens_w, save_path):
        ...
        # Save decoded (pre-refiner) image
        decoded_path = save_path.replace(".png", "_decoded.png")
        if decoded_tensors and tensor2pil is not None:
        ...
        elif decoded_tensors:
            # Fallback: save tensor directly as image
            import torchvision.transforms.functional as TF
            from PIL import Image
            t = decoded_tensors[0].squeeze(0).float().clamp(0, 1)
            img = TF.to_pil_image(t)
            img.save(decoded_path)
            print(f"Decoded image (raw) saved to {decoded_path}")

        # Try refiner pipeline if available
        try:
            if de_transform is None or RefinerImageProcessor is None or self._refiner_pipeline is None:
                print("Refiner not available, using decoded image only")
                return [decoded_path]
            ...
            import os
            refiner_steps = int(os.environ.get('REFINER_STEPS', '10'))
...
class VisionAttention(nn.Module):
    ...
    def forward(
        ...
        query_states = self.q_proj(hidden_states)  # B1: SDPA/flash path scales internally (softmax_scale=None) -> removed double-scale
