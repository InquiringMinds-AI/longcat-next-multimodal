...
So this script now does three things beyond PASS/FAIL:
  ...
  1. prints OBJECTIVE STATISTICS for every artifact (image: size, pixel spread,
     distinct colours; audio: silence/cadence breakdown) — these are measurements,
     NOT quality judgments, chosen because they move when the known silent-
     degradation modes occur, and because each one ISOLATES a single cause. Note
     duration was tried for the TTS tail and RETIRED: it is lead + speech + trail,
     so a slow read and a padded one move it identically (see media_stats);
...
from media_stats import image_stats, audio_stats
