...
Run [Meituan **LongCat-Next**](https://huggingface.co/meituan-longcat) — an any-to-any
multimodal MoE — on a single **NVIDIA DGX Spark (GB10, `sm_121`)**, serving **every modality
through one SGLang process**:

- **text** generation
- **image** understanding & **image generation** (text → image)
- **audio** understanding & **audio generation** (voice-clone TTS)
- **video** understanding (LongCat-Next has no video generation)

Quantized to **`w8a8_int8`** (8-bit weights + per-token int8 activations) — the floor for coherent
image/audio generation; 4-bit collapses both. Footprint sits comfortably under the GB10 memory ceiling.
