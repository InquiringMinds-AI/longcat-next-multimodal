...
| 文本解码 | — | **单流 22.4 tok/s；16 并发聚合 187 tok/s（8.4×），32 并发 297 tok/s（13.4×）** | CUDA 图默认开启，max-bs 32；连续批处理 |
| 智能体逐字复现输出 | — | **约 74 tok/s**（100 % 保真） | `LCN_NGRAM=1` 投机解码；全新文本不受影响（±3 %） |
...
| 多个长会话并发 | `LCN_AGENT=1 MAX_TOTAL_TOKENS=917504 MEM_FRACTION=0.88` | **约 6 个完整 128k 上下文**的 KV 池（实测：819 020 token，稳态余量约 7.2 GB，36k-token 提示词以约 3.0k tok/s 预填充且检索准确） |
