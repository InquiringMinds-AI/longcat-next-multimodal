...
try:
    ...
    def index_first_axis(hidden_states, indices):
        """Fallback: select rows by indices."""
        return hidden_states.index_select(0, indices.to(dtype=torch.long))

    def unpad_input(hidden_states, attention_mask):
        """Fallback: remove padding based on attention mask."""
        seqlens_in_batch = attention_mask.sum(dim=-1, dtype=torch.int32)
        indices = torch.nonzero(attention_mask.flatten(), as_tuple=False).flatten()
        max_seqlen_in_batch = seqlens_in_batch.max().item()
        cu_seqlens = F.pad(torch.cumsum(seqlens_in_batch, dim=0, dtype=torch.int32), (1, 0))
        bs, seq_len = hidden_states.shape[:2]
        hidden_states = hidden_states.reshape(bs * seq_len, *hidden_states.shape[2:])
        hidden_states = hidden_states.index_select(0, indices.to(dtype=torch.long))
        return hidden_states, indices, cu_seqlens, max_seqlen_in_batch

    def pad_input(hidden_states, indices, batch_size, seqlen):
        """Fallback: re-pad hidden states using indices."""
        output = torch.zeros(batch_size * seqlen, *hidden_states.shape[1:],
                           device=hidden_states.device, dtype=hidden_states.dtype)
        output.index_copy_(0, indices.to(dtype=torch.long), hidden_states)
...
class AttnProcessorFlash2Varlen:
    ...
    def __call__(
        ...
        if flash_attn_varlen_func is not None:
            ...
            # SDPA fallback: re-pad and use standard attention
            # Re-form into [batch, heads, seq, dim] for SDPA
            q = query.transpose(1, 2)  # [B, heads, S, D]
            k = key.transpose(1, 2)
