...
# Per-forward N-gram diagnostics (norms / nonzero counts) force host syncs (.item()); off by default.
_NGRAM_DIAG = os.environ.get("LCN_NGRAM_DIAG") == "1"
...
class NgramEmbedding(torch.nn.Module):
    ...
    def _oe_embed(self, indices):
        """Over-embedding lookup. The shipped w8a8 checkpoint loads oe weights as int8
        (see weight_loader), so the int8 dequant path is the live one; BF16 is the fallback."""
    ...
    def forward(self, input_ids: torch.Tensor, forward_batch: ForwardBatch):
        ...
        # Batched per-order projection in one kernel (canonical form); equivalent to the
        # per-order loop `oe_hidden_states[_i] @ self.oe_projection[_i]`.
